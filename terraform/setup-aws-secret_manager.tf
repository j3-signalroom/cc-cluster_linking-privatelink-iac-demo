...
resource "aws_secretsmanager_secret" "sandbox_src_api_key" {
    name = "${var.confluent_secret_root_path}/schema_registry_cluster"
    description = "Schema Registry Cluster secrets for Kafka client"
}

resource "aws_secretsmanager_secret_version" "sandbox_src_api_key" {
    secret_id     = aws_secretsmanager_secret.sandbox_src_api_key.id
    secret_string = jsonencode({"schema.registry.basic.auth.credentials.source": "USER_INFO",
                                "schema.registry.basic.auth.user.info": "${module.sandbox_src_api_key_rotation.active_api_key.id}:${module.sandbox_src_api_key_rotation.active_api_key.secret}",
                                "schema.registry.url": "${data.confluent_schema_registry_cluster.sandbox_src.rest_endpoint}"})
}
